...
public:
  ...
  void make_set(const unsigned &x) { nodes.emplace_back(x); }

  unsigned find(unsigned x) {
    auto node_it =
        find_if(nodes.begin(), nodes.end(), [x](auto n) { return n.id == x; });
  ...
  void union_sets(unsigned x, unsigned y) {
    ...
    // 만약 X와 Y가 같은 트리에 있다먼 그대로 종료
    if (root_x == root_y)
      return;

    // 작은 랭크의 큰 트리를 큰 랭크의 트리 쪽으로 병홥
...
template <typename T> Graph<T> minimum_spanning_tree(const Graph<T> &G) {
  ...
  // 정점 개수에 해당하는 크기의 디스조인트-셋 자료 구조 생성 및 초기하ㅗ
  auto N = G.vertices();
  ...
  while (!edge_min_heap.empty()) {
    ...
    // 선택한 에지가 사이클을 생헝하지 않으면 해당 에지를 MST에 추가
