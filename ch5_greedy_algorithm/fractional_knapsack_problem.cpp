...
auto fill_knapsack(std::vector<Object> &objects, int knapsack_capacity) {
  ...
  // 마지막 추가한 물건에 의해 배낭 최대 허용 무게가 초과된 경우 그만큼 덜어낸다
  ...
  if (weight_of_last_obj_to_remove > 0) {
    ...
    last_object.value -=
        last_object.value_per_unit_weight * weight_of_last_obj_to_remove;
  }
...
int main(int argc, char *argv[]) {
  ...
  // 분할 가능 배낭 문제 알고리즘 실행, 배낭 최대 허용 무게는 7로 설정
