...
template <typename T> auto partition(typename std::vector<T>::iterator begin, typename std::vector<T>::iterator end)
{
    //세 개의 반복자를 생성한다
    //하나는 피벗을, 나머지 둘은 벡터의 사작과 마지막 원소를 가리킨다
    //주어진 벡터가 분할되어 피벗보다 큰 원소는 오른쪽에, 작거나 같은 원소는 왼쪽에
    ...
    while (true)
    {
        ...
        //그렇지 않으면 left)iter와 right_iter가 가리키는 원소 교환
                                 ...
                                 typename std::vector<T>::iterator pivot)
{
    ...
    while (true)
    {
        ...
        while (*right_iter >= *pivot && left_iter != right_iter)
            right_iter--;
        //만약 left_iter와 right_iter가 같으면 교환할 원소가 없음을 의미합니다.
                                                     ...
                                                     typename std::vector<T>::iterator last, size_t i)
{
    ...
    if (size > 0 && i < size)
    {
        ...
        auto median_of_medians =
            (M.size() == 1) ? M.begin() : linear_time_select<T>(M.begin(), M.end() - 1, M.size() / 2);
    ...
    return begin; // control reaches end of non-void function error 해결하기 위한 임시 코드
}
...
//실제 테스트를 수행하는 함수
{
    ...
    std::cout << "3번쨰 원소: " << *linear_time_select<int>(S1.begin(), S1.end() - 1, 3) << std::endl;
    std::cout << "5번쨰 원소: " << *linear_time_select<int>(S1.begin(), S1.end() - 1, 5) << std::endl;
    std::cout << "11번쨰 원소: " << *linear_time_select<int>(S1.begin(), S1.end() - 1, 11) << std::endl;
}
